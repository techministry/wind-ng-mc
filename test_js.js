...
var sz_icon_small = [13, 17],
    ...
    img_shadow = 'templates/basic/images/gmap/modern/shadow.png',
    img_icons =  [
        [ 'templates/basic/images/gmap/modern/nodes/ap_min.png', 'templates/basic/images/gmap/modern/nodes/ap.png' ],
        [ 'templates/basic/images/gmap/modern/nodes/backbone_min.png', 'templates/basic/images/gmap/modern/nodes/backbone.png' ],
        [ 'templates/basic/images/gmap/modern/nodes/user_min.png', 'templates/basic/images/gmap/modern/nodes/user.png' ],
        [ 'templates/basic/images/gmap/modern/nodes/disconnected_min.png', 'templates/basic/images/gmap/modern/nodes/disconnected.png' ],
        [ 'templates/basic/images/gmap/modern/nodes/home_min.png', 'templates/basic/images/gmap/modern/nodes/home.png' ]
    ];
...
function gmap_onload() {
	...
	if ('15' != '') {
		...
		var bound_sw = [37.9915, 23.7658];
		var bound_ne = [37.9915, 23.7658];
...
function gmap_refresh() {
	...
	var xml_url = "?page=gmap&subpage=xml&node=10567" +
...
function getNumberedIconURL(number) {
	var num = number;
	if (num > 300) num = "more";
	return "templates/basic/images/gmap/modern/numbers/number_" + num + ".png";
}

function getNumberedIcon(number) {
	return L.icon({
		iconUrl: getNumberedIconURL(number),
		shadowUrl: "templates/basic/images/gmap/modern/shadow.png",
		iconSize: [32, 37],
		shadowSize: [51, 37],
		iconAnchor: [16, 37],
		popupAnchor: [0, -37]
	...
	for (var i = 0; i < nodes.length; i++) {
		...
		if (inbounds) {
			...
			var html = "<div style=\"padding-right: 15px; white-space: nowrap; text-align:left; font-size:12px;font-weight:bold;\"><img src=\"" + icon_s.options.iconUrl + "\" alt=\"\" />" + node_name + " (#" + node_id + ") <br />[" + node_community + "]</div><br />" +
				"<div style=\"padding-right: 15px; white-space: nowrap; text-align:left; font-size:10px;\">: " + node_adminowner + "<br />" +
				node_area + "<br />" +
				"Links: " + (parseInt(node_p2p) + parseInt(node_client_on_ap)) + " (+" + node_aps + " Access Points)" + "<br />" +
				"Clients: " + node_clients + "<br />" + ": " + node_freeifs + "<br /><br />" + "<a href=\"" + node_url + "\">Node Page</a>";
			
			if (selected[0]) {
				var this_node = selected[0].getAttribute("id");
				if (this_node != node_id) {
					html = html + "<br /><a href=\"\" onclick=\"javascript: t = window.open('?page=nodes&subpage=plot_link&a_node=" + selected[0].getAttribute("id") + "&b_node=" + node_id + "', 'popup_plot_link', 'width=600,height=450,toolbar=0,resizable=1,scrollbars=1'); t.focus(); return false;\">Line of sight (LOS)</a></div>";
				}
			} else {
				html = html + "</div>";
			}
