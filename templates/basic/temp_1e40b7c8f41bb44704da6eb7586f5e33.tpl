...
{% foreach from=data item=field key=d %}
	...
	{elseif $field.Type == 'pickup'}
		...
		<td class="table-form-field" >
			...
			{% include "generic/link.tpl"content=$lang.change onclick="javascript: t=window.open('`$field.Pickup_url`', 'popup_pickup', 'width=700,height=600,toolbar=0,resizable=1,scrollbars=1'); t.focus(); return false;" %}
			{% if $$field.Null == 'YES' %}{% include "generic/link.tpl"content=$lang.delete onclick="javascript: `$field.fullField`.value = ''; `$field.fullField`_output.innerText = ''; return false;" %}{% endif %}
		</td>	
	{elseif $field.Type == 'pickup_multi'}
		...
		<td class="table-form-field" >
			...
			{% include "generic/link.tpl"content=$lang.add onclick="javascript: t=window.open('`$field.Pickup_url`', 'popup_pickup', 'width=700,height=600,toolbar=0,resizable=1,scrollbars=1'); t.focus(); return false;" %}
			{% include "generic/link.tpl"content=$lang.remove onclick="javascript: remove_selected(window.document.`$extra_data.FORM_NAME`.elements['`$field.fullField`[]']); return false;" %}
		</td>	
